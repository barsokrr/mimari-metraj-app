"""
Fi-le Metraj Pro v1.0
Mimari Metraj ve Keşif Sistemi
...
class FiLeMetrajMotoru:
    ...
    # Çevre Şehircilik Poz Kodları (2024 Örnek)
    POZ_KUTUPHANESI = {
    ...
    def katman_analiz(self, doc) -> Dict:
    ...
    def kesif_raporu_olustur(self, katman_analiz: Dict, kat_yuksekligi: float = 3.0) -> pd.DataFrame:
        ...
        for katman_adi, veri in katman_analiz.items():
            ...
            if birim == 'm²':
                if 'duvar' in kategori['kategori']:
                    miktar = (veri['toplam_uzunluk'] / 2) * kat_yuksekligi
                else:
                    miktar = veri['toplam_alan']
            elif birim == 'm³':
                miktar = veri['toplam_alan'] * 0.2
            elif birim == 'm':
        ...
        with st.form("login"):
            st.markdown("""
                <div style="text-align: center; padding: 2rem;">
                    <h3>Giriş Yap</h3>
                    <p style="color: #888;">Barış Öker - Fi-le Yazılım A.Ş.</p>
                </div>
            """, unsafe_allow_html=True)
...
def main_app():
    ...
    with st.sidebar:
        ...
        kat_yuk = st.number_input("📏 Kat Yüksekliği (m)", value=3.0, step=0.1, min_value=1.0, max_value=20.0)
    ...
    # Ana ekran
    st.markdown('<h1 class="main-header">🏗️ Fi-le Metraj Pro</h1>', unsafe_allow_html=True)
    ...
    try:
        ...
        st.success(f"✅ {len(analiz)} katman analiz edildi | {uploaded.name}")
        ...
        st.dataframe(df_katman, use_container_width=True, hide_index=True)
        ...
        keşif_df = motor.kesif_raporu_olustur(analiz, kat_yuk)
        
        if not keşif_df.empty:
            # Birim çevirimi uygula
            if birim != 'm':
                for idx, row in keşif_df.iterrows():
                    if row['Birim'] in ['m', 'm²', 'm³'] and row['Poz No'] != '':
                        keşif_df.at[idx, 'Miktar'] = round(row['Miktar'] / birim_carpani, 2)
                        keşif_df.at[idx, 'Tutar'] = round(keşif_df.at[idx, 'Miktar'] * row['Birim Fiyat'], 2)
            
            # Toplam satırını güncelle
            toplam_tutar = keşif_df[keşif_df['Poz No'] != '']['Tutar'].sum()
            keşif_df.loc[keşif_df['Poz No'] == '', 'Tutar'] = toplam_tutar
            ...
            # Toplam göster
            st.metric("💰 Genel Toplam", f"{toplam_tutar:,.2f} TL")
            ...
            with col1:
                ...
                st.download_button(
                    "📥 Excel (CSV) İndir",
                    csv,
                    f"kesif_{uploaded.name.replace('.dxf', '')}.csv",
                    mime="text/csv",
                    use_container_width=True
                )
            with col2:
                ...
                st.download_button(
                    ...
                    f"rapor_{uploaded.name.replace('.dxf', '')}.json",
                    mime="application/json",
                    use_container_width=True
                )
        else:
            st.warning("⚠️ Keşif raporu oluşturulamadı. Katman eşleştirmelerini kontrol edin.")
