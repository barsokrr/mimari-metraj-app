...
def calculate_metrics(msp, wall_l, floor_l, door_l, win_l, h, unit_divider):
    ...
    # Adet Bazlı Sayımlar
    results["d_count"] = len(msp.query(f'*[layer=="{door_l}"]'))
    results["w_count"] = len(msp.query(f'*[layer=="{win_l}"]'))
...
# --- 4. ANA PROGRAM ---
    ...
    with st.sidebar:
        # Profil Alanı Güncellendi: Barış Öker
        ...
        # Katman ve Birim Ayarları (Tek Kutuda)
        with st.expander("📂 Katman ve Birim Ayarları", expanded=True):
            if uploaded:
                with tempfile.NamedTemporaryFile(delete=False, suffix=".dxf") as tmp:
                    tmp.write(uploaded.getbuffer())
                    doc, msp, all_layers = get_dxf_data(tmp.name)
                ...
                st.warning("Lütfen bir DXF dosyası yükleyin.")
    ...
    if uploaded and 'msp' in locals():
        ...
        df = pd.DataFrame([
            {"İmalat Kalemi": "Tuğla Duvar Örülmesi", "Miktar": round(res['w_area'], 2), "Birim": "m²"},
            {"İmalat Kalemi": "Zemin Kaplama İmalatı", "Miktar": round(res['f_area'], 2), "Birim": "m²"},
            {"İmalat Kalemi": "Kapı Montaj İşleri", "Miktar": res['d_count'], "Birim": "Adet"},
            {"İmalat Kalemi": "Doğrama ve Cam İşleri", "Miktar": res['w_count'], "Birim": "Adet"}
        ])
        st.table(df)
        os.remove(tmp.name)
    else:
        st.info("Hesaplamaları başlatmak için lütfen sol menüden DXF dosyasını yükleyin.")
